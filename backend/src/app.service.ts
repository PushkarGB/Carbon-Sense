...
export class AppService {
  ...
  async handleAppOpen(userId: Types.ObjectId) {
    ...
    const yesterdayYmd = getDateStringInTimeZone(
      yesterdayTemp,
      INDIA_TIME_ZONE,
    );
    const previousStreakDays = userProfile.streak_days ?? 0;
    const streakLost =
      previousStreakDays > 0 &&
      userProfile.last_streak_update !== UNSET_PROFILE_DATE &&
      userProfile.last_streak_update !== yesterdayYmd;

    let newStreak = 1;
    if (userProfile.last_streak_update === yesterdayYmd) {
      newStreak = previousStreakDays + 1;
    }
